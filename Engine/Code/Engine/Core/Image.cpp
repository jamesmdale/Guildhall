...
Image::Image( const std::string& imageFilePath )
{
	...
	TODO("Double check to make sure this formula is correct.  Otherwise seems to be slightly broken.");
	for(int texelIndex = 0; texelIndex < numTexels; texelIndex++ )// componentIndex += numComponents)
	{
