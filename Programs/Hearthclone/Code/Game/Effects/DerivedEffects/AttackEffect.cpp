...
AttackEffect::AttackEffect(Character* attackingWidget)
{
...
void AttackEffect::UpdateInput()
{
	...
	else if (theInput->WasKeyJustPressed(theInput->MOUSE_LEFT_CLICK))
	{
		...
		if (selectedWidget != nullptr)
		{
			...
			if (minion->m_controller == ENEMY_PLAYER_TYPE)
			{
				TODO("HANDLE TAUNT CONDITIONS");

				std::map<std::string, std::string> parameters = { {"attackerIndex", Stringf("%i", m_attackingWidget->m_characterId)}, {"targetIndex", Stringf("%i", minion->m_characterId)} };
...
void AttackEffect::RefreshTargetRenderable()
{
